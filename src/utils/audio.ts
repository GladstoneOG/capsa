class SoundSynthesizer {
  ...
  public playSlingshot() {
    ...
    // High frequency release whistle
    const osc2 = ctx.createOscillator();
    const gain2 = ctx.createGain();
    osc2.type = 'sine';
  ...
  public playHit() {
    ...
    // Crack sound using short white noise
  ...
  public playSumoDrag() {
    ...
    // Small click for tension feedback
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.type = 'triangle';
  ...
  public playSumoLaunch() {
    if (this.isMuted) return;
    const ctx = this.initCtx();
    const now = ctx.currentTime;

    // Fast whoosh/launch snap
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.type = 'sawtooth';
    osc.frequency.setValueAtTime(100, now);
  ...
  public playSumoClash() {
    if (this.isMuted) return;
    const ctx = this.initCtx();
    const now = ctx.currentTime;

    // Wood/rubber heavy thud
    const osc = ctx.createOscillator();
    const gain = ctx.createGain();
    osc.type = 'triangle';
  ...
  public playSumoSplash() {
    if (this.isMuted) return;
    const ctx = this.initCtx();
    const now = ctx.currentTime;

    // Falling void whoosh + splash noise
