...
export function sortCards(cards, method = 'rank') {
...
export function getConsecutiveStraight(cards) {
    if (cards.length !== 5)
        return null;
...
export function getStraightHighCard(cards) {
...
export function checkCombination(cards) {
...
function compareSameFiveCard(a, b) {
    const type = a.type;
...
export function canBeat(nextPlay, prevPlay, rules = { enableBombsSingle: true, enableBombsPair: true }) {
    if (nextPlay.type === 'invalid')
        return false;
    ...
    if (prevVal <= 3) {
        if (nextPlay.type !== prevPlay.type)
            return false;
    ...
    if (prevVal >= 4 && prevVal <= 8) {
        if (nextVal < 4 || nextVal > 8)
            return false; // Must be a 5-card combo
        if (nextVal > prevVal) {
...
function findCombinationsOfSize(hand, size) {
    const groups = {};
    for (const c of hand) {
        if (!groups[c.rank])
            groups[c.rank] = [];
...
function findFiveCardHands(hand) {
    const results = [];
...
export function getBotPlay(hand, prevPlay, isFirstPlay, rules = { enableBombsSingle: true, enableBombsPair: true }) {
