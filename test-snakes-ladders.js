...
export function createNewRoom(roomCode, hostName, avatar, sessionId) {
    return {
        code: roomCode,
...
export function movePlayerOnBoard(room, playerIndex, roll) {
...
class SocketClientHelper {
    ...
    constructor(url, options) {
        ...
        this.socket.on('start-error', (msg) => {
            this.errors.push(msg);
    ...
    async waitForEvent(name, timeout = 2000) {
...
async function setupLobby(url, p2 = false, bot = false, autoReady = true) {
...
testCases.push({
    ...
    run: async ({ port, forceFail }) => {
        ...
        const hostPlayer = update.players.find((p) => p.isHost);
        const pos = hostPlayer.position;
        const roll = pos - 1; // since start is pos 1
        if (forceFail) {
            if (roll >= 1 && roll <= 6)
                throw new Error('Simulated failure: roll is valid!');
        }
        else {
            if (roll < 1 || roll > 6) {
...
testCases.push({
    ...
    run: async ({ port }) => {
        ...
        host.emit('snakes-ladders-action', { roomCode: host.room.code, action: 'roll-dice', forceRoll: 3 });
        const update = await host.waitForEvent('room-updated');
        const p = update.players.find((pl) => pl.isHost);
        if (p.position !== 4) {
...
testCases.push({
    ...
    run: async ({ port }) => {
        ...
        const hostInGuestView = update.players.find((pl) => pl.isHost);
...
testCases.push({
    ...
    run: async ({ port }) => {
        ...
        const update = await host.waitForEvent('room-updated');
        const hostPlayer = update.players.find((pl) => pl.isHost);
...
testCases.push({
    ...
    run: async ({ port }) => {
        ...
        host.emit('snakes-ladders-action', { roomCode: host.room.code, action: 'roll-dice', forceRoll: 15, forcePosition: 1 });
        const update = await guest.waitForEvent('room-updated');
        const hostInGuest = update.players.find((pl) => pl.isHost);
...
testCases.push({
    ...
    run: async ({ port }) => {
        ...
        const bot = update.players.find((p) => p.isBot);
        if (bot.position === 1) {
    ...
    testCases.push({
        ...
        run: async ({ port }) => {
            ...
            if (id === 'TC-F1-08') {
                ...
                const hostPlayer = rejoinedRoom.room.players.find((p) => p.sessionId === 'h-sess');
                ...
                const newHost = guestUpdate.players.find((p) => p.name === 'Guest');
                if (!newHost.isHost) {
                ...
                testRoom.players.push({ id: 'bot1', name: 'Bot 1', avatar: null, isHost: false, isReady: true, isBot: true, position: 1, consecutiveSixes: 0 }, { id: 'bot2', name: 'Bot 2', avatar: null, isHost: false, isReady: true, isBot: true, position: 1, consecutiveSixes: 0 }, { id: 'bot3', name: 'Bot 3', avatar: null, isHost: false, isReady: true, isBot: true, position: 1, consecutiveSixes: 0 });
                let turnCount = 0;
                ...
                if (testRoom.gameState !== 'gameover' && turnCount >= 500) {
...
async function main() {
    ...
    if (forceFail)
        console.log('WARNING: Force-fail mode active. Test suite is expected to fail.');
    console.log('==================================================');
    let server = null;
    const port = 3005;
    if (isVerifyRunner) {
